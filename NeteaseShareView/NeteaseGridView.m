...
#import "NeteaseGridViewRow.h"

@interface NeteaseGridView ()
@property (nonatomic, readwrite) NSInteger numOfRows;
@end

@implementation NeteaseGridView
...
{
	NeteaseGridViewCell* result = _reusableCell;
	_reusableCell = nil;
	return result;
}
...
- (NeteaseGridViewCell *) getGridViewCellWithGridIndex:(NeteaseGridViewIndex *)gridIndex
{
    ...
    if (gridIndex.rowIndex < rows)
    {
        ...
        NeteaseGridViewRow *gridViewRow = (NeteaseGridViewRow *)[self cellForRowAtIndexPath:indexPath];
...
- (void)selectGridViewCellAtGridIndex:(NeteaseGridViewIndex *)gridIndex
{
    self.selectedIndex = [NeteaseGridViewIndex gridViewIndexWithRow:gridIndex.rowIndex column:gridIndex.columnIndex];
    NeteaseGridViewCell *cell = [self getGridViewCellWithGridIndex:gridIndex];
    if (cell)
    {
        cell.bSelected = YES;
    }
...
- (void)deselectGridViewCellAtGridIndex:(NeteaseGridViewIndex *)gridIndex
{
    NeteaseGridViewCell *cell = [self getGridViewCellWithGridIndex:gridIndex];
    if (cell)
    {
...
{
    static NSString *CellIdentifier = @"NeteaseGridViewRow";
	
    NeteaseGridViewRow *gridViewRow = (NeteaseGridViewRow *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (gridViewRow == nil)
    {
        gridViewRow = [[NeteaseGridViewRow alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier] ;
    }
	...
	for (NSInteger i=0; i < self.numOfColumns; i++)
    {
		...
		if ((i + indexPath.row * self.numOfColumns) >= countCells)
        {
            ...
            {
				NeteaseGridViewCell *needHideCell = ((NeteaseGridViewCell *)[gridViewRow.contentView.subviews objectAtIndex:i]);
        ...
        NeteaseGridViewIndex *index = [NeteaseGridViewIndex gridViewIndexWithRow:indexPath.row column:i];
		NeteaseGridViewCell *cell = [self.gridViewDelegate gridView:self cellForGridIndex:index];
...
#pragma mark - NeteaseGridViewCellDelegate
...
- (void)cellSelected:(NeteaseGridViewCell *)cell
{
    ...
    {
        NeteaseGridViewCell *selectedCell = [self getGridViewCellWithGridIndex:self.selectedIndex];
        if (selectedCell)
        {
            selectedCell.bSelected = NO;
        }
        self.selectedIndex = [NeteaseGridViewIndex gridViewIndexWithRow:cell.gridIndex.rowIndex column:cell.gridIndex.columnIndex];
    }
