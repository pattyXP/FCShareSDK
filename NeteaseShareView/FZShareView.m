...
#import "FZShareView.h"

#import "FZShareItemCollectionViewCell.h"

#define kMAX_CONTENT_HEIGHT 360
#define kEdgeMarginSpace 10
#define kMinimumInteritemSpacing 5
#define kUIViewAnimationTimeInterval 0.2

@interface FZShareView ()<UICollectionViewDataSource, UICollectionViewDelegate, UIGestureRecognizerDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 200)];
        ...
        CGFloat itemWidth = kShareItemWidth;
        if ([UIScreen mainScreen].bounds.size.width > 320) {
            itemWidth = 80;
        }
        layout.itemSize = CGSizeMake(itemWidth, kShareItemHeight);
        ...
        [_collectionView registerClass:[FZShareItemCollectionViewCell class] forCellWithReuseIdentifier:@"NIEShareItemCell"];
        ...
        //取消按钮与视图之间的分割线
        _cancelSeparatorView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 2)];
        _cancelSeparatorView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.35f];
...
- (void)setCancelButtonHidden:(BOOL)isHidden {
    _cancelButtonHidden = isHidden;
...
{
    FZShareItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NIEShareItemCell" forIndexPath:indexPath];

    cell.iconImageView.image = self.itemImages[indexPath.item];
    cell.titleLabel.text = self.itemTitles[indexPath.item];
...
{
    [self dismissShareView:indexPath.item];
}
...
- (void)dismissShareView:(NSInteger)index
{
    CGRect contentFrame = self.contentView.frame;
    ...
    [UIView animateWithDuration:kUIViewAnimationTimeInterval animations:^{
        ...
        if (self.shareViewDidClickItemBlock) {
            self.shareViewDidClickItemBlock(self, index);
        }
    }];
}
